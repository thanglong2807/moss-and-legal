  ...
  return (
    <div className={`relative ${className}`}>
      ...
      {open && filtered.length > 0 && (
        <div className="absolute z-50 mt-1 w-full bg-surface border border-base rounded-xl shadow-lg max-h-52 overflow-y-auto">
          {filtered.map(o => (
            <div
              key={o.id}
              className={`px-3 py-2 text-sm cursor-pointer hover:bg-orange-50 ${o.id === value ? 'bg-orange-50 font-black text-orange-700' : 'text-body'}`}
