    ...
    <div
      ...
      className={`border rounded-[28px] p-5 cursor-pointer transition-all flex flex-col h-full relative overflow-hidden group ${
        isSelected
          ...
          : 'bg-surface border-faint hover:bg-orange-50/40 hover:border-orange-200 hover:shadow-sm'
      }`}
    >
      {/* Header */}
      <div className="flex justify-between items-start mb-4">
        <div className={`w-10 h-10 rounded-2xl flex items-center justify-center shadow-sm border transition-all ${
          isSelected
            ...
            : 'bg-surface text-orange-500 border-faint group-hover:bg-orange-600 group-hover:text-white group-hover:border-orange-600'
        }`}>
          <Users size={18} />
        </div>
      ...
      <div className="flex-1">
        <div className="flex items-center gap-2 mb-1">
          ...
          <h3 className={`text-base font-black leading-tight truncate transition-colors ${
            isSelected ? 'text-orange-700' : 'text-strong group-hover:text-orange-600'
          }`}>{customer.name}</h3>
        </div>
        <p className="text-weak font-bold text-xs mb-3 flex items-center gap-1.5">
          <Phone size={12} className="text-weak shrink-0" />
          {customer.phone}
        </p>
        <div className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest text-weak">
      ...
      {/* Footer */}
      <div
        className="mt-4 pt-3 border-t border-faint flex justify-between items-center"
        onClick={(e) => e.stopPropagation()}
      >
        <span className="text-[10px] font-black text-weak uppercase tracking-widest">
        ...
        <button
          ...
          className={`text-[10px] font-black uppercase tracking-widest flex items-center gap-1 transition-colors ${
            isSelected ? 'text-orange-600' : 'text-weak hover:text-orange-600'
          }`}
        >
