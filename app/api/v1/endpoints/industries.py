...
@router.get("/", response_model=List[IndustryBase], dependencies=[Depends(get_current_user)])
def list_industries(db: Session = Depends(get_db)):
    return industry_service.get_all(db)

@router.post("/", response_model=IndustryBase, dependencies=[Depends(require_permission("config", "create"))])
def create_industry(data: IndustryBase, db: Session = Depends(get_db)):
    return industry_service.create(db, data)

@router.put("/{industry_id}", response_model=IndustryBase, dependencies=[Depends(require_permission("config", "update"))])
...
@router.delete("/{industry_id}", dependencies=[Depends(require_permission("config", "delete"))])
